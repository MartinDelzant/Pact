...
public class Block {
	...
	public float SousEchantillonMAD(Block anOtherBlock)
			throws IndexOutOfBoundsException {
		if (anOtherBlock.hauteur != this.hauteur
				|| anOtherBlock.largeur != this.largeur) {
			return -1;
		}
		float res = 0;
		for (int i = 0; i < this.hauteur; i += 2) {
			for (int j = 0; j < this.largeur; j += 2) {
				res += Math.abs(anOtherBlock.getValue(i, j)
						- this.getValue(i, j));
			}
		}
		return (res / (hauteur * largeur));
