...
//				long[] buffer = new long[BUF_SIZE * numChannels];
	...
	/*
	 *Librement inspiré du fichier WriteExample.java
	 */
	public static void createWaveFile(String fileName, int channels,
			int sampleRate, double duration, float[] frequencies) throws IOException,
			WavFileException {

		if(frequencies.length != channels){
			System.out.println("Impossible ici");
			return ;
		}
		...
		WavFile wavFile = newWavFile(new File(fileName), channels, numFrames,
				16, sampleRate);
		...
		while (frameCounter < numFrames) {
			...
			for (int s = 0; s < toWrite; s++, frameCounter++) {
				for (int i = 0; i<frequencies.length; i++){
					//TODO : Check
					buffer[i][s] = Math.sin(2.0 * Math.PI* frameCounter * frequencies[i] / sampleRate) ;
				}
			}
